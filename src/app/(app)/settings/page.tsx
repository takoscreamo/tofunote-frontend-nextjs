...
import { fetcherGet, fetcherDelete, fetcherPost } from "@/fetch/fetcher";
...
const SettingsPage = () => {
  ...
  const handleGoogleLink = async () => {
    setLinkLoading(true);
    try {
      const { data, err } = await fetcherPost<{ auth_url: string }>(EP.google_link());
      if (err || !data?.auth_url) {
        setError("Google連携の開始に失敗しました");
  ...
  return (
    <div className="max-w-xs w-full mx-auto py-2 px-2">
      ...
      <hr className="my-6" />
      <section className="mb-2">
        <h2 className="text-base font-semibold text-gray-800 mb-1">アカウント連携</h2>
        <p className="text-xs text-gray-500 mb-3">
          Googleアカウントと連携すると、別の端末でも同じデータにアクセスできます。
        </p>
        {isGoogleLinked ? (
          <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded">
            ...
            <span className="text-sm text-green-700 font-medium">Googleアカウントと連携済み</span>
          </div>
        ) : (
          <button
            type="button"
            ...
            className="w-full py-2 text-sm bg-white hover:bg-gray-50 text-gray-700 rounded border border-gray-300 font-semibold transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            ...
            {linkLoading ? '処理中...' : 'Googleアカウントと連携'}
          </button>
        )}
      </section>
    </div>
